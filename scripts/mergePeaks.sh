...
peaksFile=${1} # /media/rad/HDD1/atacseq/anja/tALLcellLineMm/analysis/mergedReps/peaksFileList.txt
out=${2}       # /media/rad/HDD1/atacseq/christine/AGRad_ATACseq_MUC001/analysis/5320_53631_53646_6075_merge_master_peaks.bed

# Read a file's content (with spaces) into an array
# Source: https://stackoverflow.com/questions/2389137/read-a-files-content-with-spaces-into-an-array
OLDIFS="$IFS"
IFS=$'\n'
summit_bed=()
while read line
do
  summit_bed=("${summit_bed[@]}" "$line")
done < ${peaksFile}
IFS="$OLDIFS"
...
echo ""
echo "Final merged peaks files is: ${out}"
exit 0
