...
echo "macs2 callpeak --name ${bname} --treatment ${trtBam} --outdir ${peaksoutdir} --format BAM --pvalue 1e-5 --gsize ${spc} --broad -q 0.05 --extsize 200 --nomodel --shift -100 --nolambda --keep-dup all 2>&1 | tee -a ${logFile}" >> "${scriptFile}"
echo "" >> "${scriptFile}"
